...
let partial_function loc () =
  ...
  Lprim(Praise Raise_regular, [Lprim(Pmakeblock(0, Immutable, Error "matching partial_function"),
...
let do_for_multiple_match loc paraml pat_act_list partial =
  ...
  let raise_num,pm1 =
    ...
    | Partial ->
        ...
        raise_num,
        { cases = List.map (fun (pat, act) -> ([pat], act)) pat_act_list;
          args = [Lprim(Pmakeblock(0, Immutable, Error "do_for_multiple_match Partial"), paraml), Strict] ;
          default = [[[omega]],raise_num] }
    | _ ->
        -1,
        { cases = List.map (fun (pat, act) -> ([pat], act)) pat_act_list;
          args = [Lprim(Pmakeblock(0, Immutable, Error "do_for_multiple_match fallback"), paraml), Strict] ;
          default = [] } in
