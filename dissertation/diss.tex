...
Word Count:         & \bf 11248\footnotemark[1] \\
...
which is a pseudo-expression representing a list of statements. The
...
Note that this does not work if the function is not closed (depends on values
...
In my scheme, each exception handler calls \lstinline!setjmp!, and pushes its
\lstinline!jmpbuf! onto the head of a global singly-linked list.
...
that was raised is stored in another global variable, so that the
...
int f(int x) {
  ...
  tail call position.  The C specification does not provide any guarantees
  about tail-calls, however it is still somewhat surprising that GCC fails to recognise
  this case.
