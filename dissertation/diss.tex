...
Unix-like environment is required, with the \lstinline!liballocs! library. TODO Elaborate?
...
datatype in my compiler. This type is required to represent polymorphic OCaml types: for instance a function with a parameter of OCaml type \lstinline!'a! would take a C parameter of type \lstinline!ocaml_value_t!.\footnote{As it turns out, it's easier to represent \textit{all} OCaml values as values of this type, not just polymorphic ones.}
...
\begin{enumerate}
  ...
  \item \textbf{NaN boxing}, as used by WebKit's JavaScriptCore engine,
    SpiderMonkey and LuaJIT. This is the scheme I chose to use, and is
...
boxing was inspired by JavaScriptCore's (TODO: cite) however the exact details
are original.
...
particular, any number with an exponent of \texttt{0x7FF} and non-zero mantissa
is a NaN value. There are only two different types of NaNs: quiet (qNaN) and
signalling (sNaN). Modern CPUs only ever use one specific (canonical) bit
representation for each type of NaN, which gives us the entire rest of the
range to embed other types of values into.
...
Using this representation \lstinline!ocaml_value_t! becomes
(conceptually) a tagged union of the following:
\begin{enumerate}
...
Logical right shift is relatively simple -- first set the top 14 bits to zero by using a mask, then perform a native-width logical right shift.

Arithmetic right and division are trickier. We first need to ``clean up'' the top 14 bits by extending our real sign bit (bit 49) into them. This can be achieved using the following C bitfield trick:
\begin{lstlisting}
...
The compiler will probably implement this as \lstinline!(x lsl 14) asr 14!.}
...
\begin{enumerate}
  ...
  \item If source is \lstinline!C_Value! and target is not, then we need to
    unbox the expression. Assuming the cast is valid, then 
    that
\end{enumerate}
